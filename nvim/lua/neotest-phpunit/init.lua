...
function NeotestAdapter.build_spec(args)
  ...
  if vim.fn.filereadable("bin/dev") then
    binary = {"bin/dev", "test"}
  end

  local cwd = NeotestAdapter.root(position.path)
  local ppath = path:new(position.path):make_relative(cwd)

  local command = vim.tbl_flatten({
    binary,
    position.name ~= "tests" and ppath,
